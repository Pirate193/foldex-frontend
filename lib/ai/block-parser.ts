/**
 * AI Block Parser — Client-side port of backend/src/lib/ai-block-parser.ts
 * 
 * Handles:
 * - Zod schemas for structured AI output (note blocks)
 * - Converting AI schema output → BlockNote format
 * - Converting BlockNote → Markdown (for update context)
 * 
 * Difference from backend: uses crypto.randomUUID() instead of Node's randomUUID
 */
...
// ========================================
// 4. BLOCKNOTE → MARKDOWN CONVERSION
// ========================================
...
function blockToMarkdown(block: BlockNoteBlock): string | null {
  switch (block.type) {
    ...
    case 'codeBlock': {
      ...
      const code = Array.isArray(block.content) ? block.content.map((c: any) => c.text).join('') : '';
      return `\`\`\`${language}\n${code}\n\`\`\``;
    }
...
function contentToMarkdown(content: any[]): string {
  ...
  return content.map((segment: any) => {
