 ...
 * Mirrors backend/src/lib/manim-agent.ts but runs in the browser
...
export async function generateManimCode({
    ...
    const selectorResult = await generateObject({
        model,
        schema: skillSelectorSchema,
    ...
    const selectedSkillsText = loadSkills(selectorResult.object.selectedSkills);
