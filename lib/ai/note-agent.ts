/**
 * Note Agent — Client-side port of backend/src/lib/noteagent.ts
 * 
 * Sub-agent that generates rich, structured note content using AI.
 * Uses generateText with structured output (Zod schema) to produce
 * BlockNote-compatible blocks.
 */

import { z } from "zod";
import { blocksSchema, BlockContentSchema } from "./block-parser";
...
`;
  ...
  console.log('[NoteAgent] Creating note content for:', topic);
  ...
  console.log('[NoteAgent] Generated', output.length, 'blocks');
  return output;
}
...
`;
  ...
  console.log('[NoteAgent] Updating note, instructions:', updateInstructions);
  ...
  console.log('[NoteAgent] Updated to', output.length, 'blocks');
  return output;
}
