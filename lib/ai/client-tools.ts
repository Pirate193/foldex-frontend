/**
 * Client-Side AI Tools — Port of backend/src/lib/tools.ts
 * 
 * These tools operate on the LOCAL SQLite database (via localnotes/localfolders)
 * and fire-and-forget cloud mirror calls for logged-in users.
 * 
 * Used by the client-side AI transport on desktop.
 */
...
import { mirrorCreateNote, mirrorUpdateNote, mirrorCreateFolder, mirrorUpdateFolder } from "@/lib/services/cloud-mirror";
    ...
    updateNote: tool({
      ...
      execute: async ({
        ...
        try {
          ...
          // Fire-and-forget cloud mirror
          mirrorUpdateNote(noteId, { content: blockNoteContent });
    ...
    updateFolder: tool({
      ...
      execute: async ({ folderId, name, parentId }) => {
        try {
          ...
          // Fire-and-forget cloud mirror
          mirrorUpdateFolder(folderId, { name, parentId });
