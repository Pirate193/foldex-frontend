...
import { localUser, userSettings } from "../schema.local";
...
${websearch ? 'The user has a tavily api key so you can use it' : '\n- The user has no tavily api key so dont use web search tool'}
...
export const DEFAULT_SYSTEM_PROMPT = buildSystemPrompt({ name: "Student" });
...
export async function getEffectiveSystemPrompt(
    ...
    try {
        const [settings] = await db.select().from(userSettings);
        if (settings) systemPrompt = settings.systemPrompt || undefined;
        
        const [user] = await db.select().from(localUser);
        if (user && user.name) userName = user.name;
    } catch (e) {
        console.error("Error reading user settings for AI prompt", e);
    }
