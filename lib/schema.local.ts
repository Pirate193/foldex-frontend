...
// Tracks which local items have been pushed to the cloud.
// Because the backend auto-generates UUIDs, local IDs ≠ cloud IDs.
export const syncMap = sqliteTable("sync_map", {
    localId: text("localId").notNull(),
    cloudId: text("cloudId").notNull(),
    tableName: text("tableName").notNull(), // "folders" | "notes" | "chats" | "messages" | "templates"
    syncedAt: integer("syncedAt", { mode: "timestamp_ms" }).$defaultFn(() => new Date()).notNull(),
}, (table) => ({
    syncMapLocalIdx: index("sync_map_local_idx").on(table.localId, table.tableName),
    syncMapCloudIdx: index("sync_map_cloud_idx").on(table.cloudId, table.tableName),
}));
