...
export const getLocalDb = async () => {
  ...
  `);
  await tauriDb.execute(`CREATE INDEX IF NOT EXISTS sync_map_local_idx ON sync_map(localId, tableName)`);
  await tauriDb.execute(`CREATE INDEX IF NOT EXISTS sync_map_cloud_idx ON sync_map(cloudId, tableName)`);
